...
func play_audio_safely(node_name: String, property_name: String):
	var audio := get_node_or_null("/root/AudioManager")
	if audio == null:
		return

	if audio.has_node(node_name):
		audio.get_node(node_name).play()
	elif property_name in audio:
		audio.get(property_name).play()
...
func _on_Attack_pressed():
	...
	if current_player_time >= 0:
		...
		play_audio_safely("hurtSfx", "hurt_sfx")
...
func _on_Isolate_pressed():
	...
	if current_player_time >= 0:
		play_audio_safely("isolateSfx", "isolate_sfx")
