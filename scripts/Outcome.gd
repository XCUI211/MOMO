...
@onready var restart_boss_button: Button = $CanvasLayer/RestartBossButton

var current_body_text := ""
var is_typing := false
var current_outcome_key := ""

func _ready() -> void:
	_apply_fonts()
	restart_button.pressed.connect(_on_restart_pressed)
	restart_boss_button.pressed.connect(_on_restart_boss_pressed)
	current_outcome_key = _determine_outcome()
	_setup_restart_buttons(current_outcome_key)
	_display_outcome(current_outcome_key)
...
func _apply_fonts() -> void:
	...
	restart_boss_button.add_theme_font_override("font", OUTCOME_FONT)
	restart_boss_button.add_theme_font_size_override("font_size", 33)
	restart_boss_button.add_theme_color_override("font_color", TEXT_COLOR)
	restart_boss_button.add_theme_color_override("font_outline_color", OUTLINE_COLOR)
	restart_boss_button.add_theme_constant_override("outline_size", OUTLINE_SIZE)
...
func _determine_outcome() -> String:
	...
	if v < THRESHOLD_LOW:
		...
		return "win_b"
	return "win_c"
func _setup_restart_buttons(key: String) -> void:
	restart_button.visible = false
	restart_button.disabled = true

	restart_boss_button.visible = false
	restart_boss_button.disabled = true

	if key == "defeat":
		restart_button.text = "Restart"
	else:
		restart_button.text = "Communication Restart"
		restart_boss_button.text = "Fight Restart"
		
func _show_restart_buttons() -> void:
	restart_button.visible = true
	restart_button.disabled = false

	if current_outcome_key != "defeat":
		restart_boss_button.visible = true
		restart_boss_button.disabled = false
...
func finish_typing() -> void:
	is_typing = false
	body_label.text = current_body_text
	_show_restart_buttons()
...
func _on_restart_pressed() -> void:
	...
	if current_outcome_key == "defeat":
		GlobalState.reset_for_dialogue(1)
	else:
		GlobalState.reset_for_dialogue(0)

	get_tree().change_scene_to_file(MAIN_SCENE)


func _on_restart_boss_pressed() -> void:
	stop_all_bgm()
	GlobalState.reset_for_boss(0)
